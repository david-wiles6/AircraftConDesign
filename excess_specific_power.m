...
for i = 1:length(altitude)
    for j = 1:length(M)
        ...
        D_1g(i, j) = dragCalc(altitude(i), M(j), S, 1, W, AR);
        D_5g(i, j) = dragCalc(altitude(i), M(j), S, 5, W, AR);
        ...
        Spex_1g_Max_TO(i, j) = V(i, j)*(TAB(i,j)/4.448*cosd(alpha_min(i, j))-D_1g(i, j))/W_TO;
    end
end
